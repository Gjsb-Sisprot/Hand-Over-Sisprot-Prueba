...
import {
  Save, Plus, Trash2, CalendarDays, Headphones, MapPin, Building2,
...
const VENEZUELA_HOLIDAYS: { [key: string]: string } = {
  "0-1": "AÃ±o Nuevo",
  "3-19": "ProclamaciÃ³n de la Independencia",
  "4-1": "DÃ­a del Trabajador",
  "5-24": "Batalla de Carabobo",
  "6-5": "DÃ­a de la Independencia",
  "6-24": "Natalicio del Libertador",
  "9-12": "DÃ­a de la Resistencia IndÃ­gena",
  "11-24": "VÃ­spera de Navidad",
  "11-25": "Navidad",
  "11-31": "Fin de AÃ±o"
};
...
export default function GuardiasPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const fetchedAgents: { id: string, name: string }[] = resAgents.data || [];
  ...
  const { hasRegularWeekday, weekendDatesText } = useMemo(() => {
    if (!selectedWeek) return { hasRegularWeekday: false, weekendDatesText: "" };
    ...
    for (let d = selectedWeek.startDay; d <= selectedWeek.endDay; d++) {
      ...
      if (dayOfWeek < 4) hasRegular = true;
      if (dayOfWeek === 4 && !selectedWeek.isSpecial) hasRegular = true;
      if (dayOfWeek >= 5) weekendDays.push(d.toString().padStart(2, '0'));
    }
    return { 
      hasRegularWeekday: hasRegular, 
      weekendDatesText: weekendDays.length > 0 ? `${weekendDays.join('/')}-${monthStr}` : "" 
    };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      sortedWeeks.forEach((group, idx) => {
        ...
        // Determinar límites de la semana
        const sortedItems = group.sort((a, b) => a.startDay - b.startDay);
        const minDay = sortedItems[0].startDay;
        const maxDay = sortedItems[sortedItems.length - 1].endDay;
        
        const dMin = new Date(currentYear, currentMonth, minDay);
        const dMax = new Date(currentYear, currentMonth, maxDay);
        const startIdx = (dMin.getDay() + 6) % 7;
        const endIdx = (dMax.getDay() + 6) % 7;

        let leftCol = `SEMANA DEL ${minDay.toString().padStart(2, '0')} AL ${maxDay.toString().padStart(2, '0')}/${monthStr}\n\n`;
        let rightCol = "";
        const dateCol = `${minDay.toString().padStart(2, '0')}-${maxDay.toString().padStart(2, '0')}\n${monthStr}/${currentYear}`;

        // --- CONSTRUIR COLUMNA IZQUIERDA (Días Laborales) ---
        ...
        if (lvItems.length > 0) {
          ...
          if (holidaysLV.length > 0) {
            // Caso mixto: Laborales + Feriado
            const startDayName = WEEKDAYS[(new Date(currentYear, currentMonth, minDay).getDay() + 6) % 7];
            const holiday = holidaysLV[0];
            const hIdx = (new Date(currentYear, currentMonth, holiday.startDay).getDay() + 6) % 7;
            const hDateStr = holiday.startDay.toString().padStart(2, '0');

            if (hIdx === 4) { // Feriado es Viernes
              leftCol += `LUNES A JUEVES: AGENCIA ABIERTA\n`;
            } else if (hIdx === 0) { // Feriado es Lunes
              leftCol += `MARTES A VIERNES: AGENCIA ABIERTA\n`;
            } else {
              leftCol += `DÍAS LABORALES: AGENCIA ABIERTA\n`;
            }

            const reg = regularLV[0] || group.find(i => i.weekCallCenterPerson);
            leftCol += `CALL CENTER: ${reg?.weekCallCenterPerson || 'SIN ASIGNAR'}\n`;
            leftCol += `SOPORTE TÉCNICO: ${reg?.weekSoportePerson || 'SIN ASIGNAR'}\n`;
            
            leftCol += `\n${holiday.specialTitle || `VIERNES ${hDateStr}/${monthStr}: DÍA FERIADO - AGENCIA CERRADA`}\n`;
            leftCol += `CC: ${holiday.specialCallCenter || '-'}\nMN: ${holiday.weekendMonitoreoPerson || '-'}\n`;
            leftCol += `SOPORTE: ${holiday.specialSoporte || '-'}`;
          } else if (regularLV.length > 0) {
            const startName = WEEKDAYS[startIdx].toUpperCase();
            const endName = WEEKDAYS[Math.min(endIdx, 4)].toUpperCase();
            leftCol += `${startName} A ${endName}: AGENCIA ABIERTA\n`;
            leftCol += `CALL CENTER: ${regularLV[0].weekCallCenterPerson || 'SIN ASIGNAR'}\n`;
            leftCol += `SOPORTE TÉCNICO: ${regularLV[0].weekSoportePerson || 'SIN ASIGNAR'}`;
          }
        } else {
          leftCol += "SOLO FIN DE SEMANA OPERATIVO";
        }

        // --- CONSTRUIR COLUMNA DERECHA (Fin de Semana) ---
        const sdItems = group.filter(i => {
          const d = new Date(currentYear, currentMonth, i.startDay);
          const day = (d.getDay() + 6) % 7;
          return day >= 5 || i.endDay >= 6;
        });

        if (sdItems.length > 0) {
          const regSD = sdItems.find(i => !i.isSpecial);
          const sat = Math.max(minDay, 6); // Simplificación
          const sun = maxDay;
          
          rightCol = `SAB - DOMING ${minDay === maxDay ? minDay : minDay + '/' + maxDay}-${monthStr}\n`;
          rightCol += `08:00 AM A 08:00 PM\n\n`;
          ...
          rightCol += `AGENCIA TURMERO: ${regSD?.weekendAgenciaPerson || 'CERRADA'}`;

          const holidaysSD = sdItems.filter(i => i.isSpecial);
          holidaysSD.forEach(h => {
            const hName = WEEKDAYS[(new Date(currentYear, currentMonth, h.startDay).getDay() + 6) % 7].toUpperCase();
            rightCol += `\n\nFERIADO ${hName} ${h.startDay}/${monthStr}:\n`;
      ...
      autoTable(doc, {
        startY: 30,
        head: [['ITEM', 'LUNES A VIERNES / FERIADOS', 'SÁBADO / DOMINGO', 'FECHA']],
        body: tableRows,
        theme: 'grid',
        headStyles: { fillColor: [30, 41, 59], textColor: 255, fontSize: 8, halign: 'center', fontStyle: 'bold' },
        styles: { fontSize: 7, cellPadding: 2, textColor: [15, 23, 42], lineWidth: 0.05, lineColor: [150, 150, 150] },
        columnStyles: {
          0: { cellWidth: 8, halign: 'center', valign: 'middle' },
          1: { cellWidth: 112 },
          2: { cellWidth: 112 },
          3: { cellWidth: 28, halign: 'center', valign: 'middle', fontStyle: 'bold' }
        },
        margin: { top: 30 },
        pageBreak: 'avoid',
        didParseCell: function(data: any) {
          if (data.section === 'body') {
            ...
            if (data.column.index === 1 && text.includes('SEMANA')) {
               data.cell.styles.fillColor = [240, 250, 240];
            }
            if (data.column.index === 2 && text.includes('SAB - DOMING')) {
      ...
      doc.text(`Sistema de Control Sisprot - Generado por: ${currentUser} - ${new Date().toLocaleString()}`, 14, finalY + 8);
  ...
  const generateFullMonth = () => {
    ...
    for (let d = 1; d <= lastDay; d++) {
      ...
      if (VENEZUELA_HOLIDAYS[key]) {
        newItems.push({
          ...
          endDay: d,
          isSpecial: true,
          specialTitle: `${WEEKDAYS[(new Date(currentYear, currentMonth, d).getDay() + 6) % 7].toUpperCase()} ${d.toString().padStart(2, '0')}/${(currentMonth+1).toString().padStart(2, '0')}: DÍA FERIADO - AGENCIA CERRADA`,
          specialCallCenter: "",
          specialSoporte: "",
    ...
    while (d <= lastDay) {
      ...
      const dayOfWeek = (date.getDay() + 6) % 7; // 0=L, 6=D
      const daysToEndOfWeek = 6 - dayOfWeek;
      const endDay = Math.min(d + daysToEndOfWeek, lastDay);
    ...
    setData(newItems);
    if (newItems.length > 0) setSelectedWeekId(newItems[0].id);
    toast.success(`${newItems.length} registros generados para ${MONTHS[currentMonth]}`);
  };
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] text-foreground font-sans flex flex-col">
      ...
      <main className="flex-1 flex overflow-hidden relative">
          ...
          isSidebarOpen ? "translate-x-0" : "-translate-x-full"
           ...
           <div className="p-4 lg:p-6 border-b border-border/40 bg-muted/20">
              <div className="flex items-center justify-between mb-4 lg:hidden">
                 <h2 className="text-xs font-black text-muted-foreground uppercase tracking-widest">PlanificaciÃ³n</h2>
              ...
              {filteredData.sort((a,b) => a.startDay - b.startDay).map((week) => (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-sm font-black text-foreground">
                         {week.isSpecial ? (week.specialTitle || `DÃ­a ${week.startDay}`) : `${week.startDay} al ${week.endDay}`}
                       </span>
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 opacity-70 group-hover:opacity-100 transition-opacity">
                       <div className="w-3 h-3 flex items-center justify-center text-[10px]">ðŸ› ï¸</div>
                       <p className="text-[10px] font-bold text-muted-foreground truncate uppercase">
        ...
        {/* Detail View */}
        <section className="flex-1 overflow-y-auto bg-white/40 custom-scrollbar">
          {selectedWeek ? (
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Planificación Activa</span>
                  </div>
              ...
              <div className="grid grid-cols-1 gap-10">
                <div className={cn(
                  ...
                  <label className="relative inline-flex items-center cursor-pointer scale-125 mr-4">
                    <input type="checkbox" checked={selectedWeek.isSpecial} onChange={() => updateItem(selectedWeek.id, 'isSpecial', !selectedWeek.isSpecial)} className="sr-only peer" />
                ...
                {selectedWeek.isSpecial && (
                  <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                    <label className="text-xs font-black text-red-600 uppercase tracking-widest ml-1">Título y Fecha del Evento</label>
                    <input className="w-full bg-red-50 border-2 border-red-100 focus:border-red-500 focus:bg-white rounded-2xl px-6 py-4 outline-none font-black text-xl text-red-900" placeholder="Ej: Miércoles 1 de Mayo: Feriado" value={selectedWeek.specialTitle} onChange={e => updateItem(selectedWeek.id, 'specialTitle', e.target.value)} />
                  </div>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="bg-[#fcfcfc] border border-border/40 rounded-[2rem] p-8 space-y-6">
                    <div className="flex items-center gap-3">
                      <div className="p-3 bg-blue-100 text-blue-600 rounded-2xl"><Headphones className="w-6 h-6" /></div>
                      <h3 className="text-xl font-black uppercase tracking-tighter">Call Center</h3>
                    </div>
                    <div className="space-y-4">
                      {hasRegularWeekday && (
                        <div className="space-y-2">
                          <p className="text-[10px] font-black text-muted-foreground uppercase">L-V (8:00 AM - 5:00 PM)</p>
                          <MultiSelect value={selectedWeek.weekCallCenterPerson} onChange={v => updateItem(selectedWeek.id, 'weekCallCenterPerson', v)} options={agents} placeholder="Sin asignar" />
                        </div>
                      )}
                      <div className="space-y-2">
                        <p className="text-[10px] font-black text-amber-600 uppercase">SÁB-DOM {weekendDatesText}</p>
                        <MultiSelect value={selectedWeek.weekendCallCenterPerson} onChange={v => updateItem(selectedWeek.id, 'weekendCallCenterPerson', v)} options={agents} placeholder="Sin asignar" />
                      </div>
                      {selectedWeek.isSpecial && (
                        <div className="space-y-2 p-4 bg-red-50 rounded-2xl border border-red-100">
                          <p className="text-[10px] font-black text-red-600 uppercase">Feriado</p>
                          <MultiSelect value={selectedWeek.specialCallCenter} onChange={v => updateItem(selectedWeek.id, 'specialCallCenter', v)} options={agents} placeholder="Sin asignar" />
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="bg-[#fcfcfc] border border-border/40 rounded-[2rem] p-8 space-y-6">
                    <div className="flex items-center gap-3">
                      <div className="p-3 bg-amber-100 text-amber-600 rounded-2xl text-xl flex items-center justify-center">???</div>
                      <h3 className="text-xl font-black uppercase tracking-tighter">Soporte Técnico</h3>
                    </div>
                    <div className="space-y-4">
                      {hasRegularWeekday && (
                        <div className="space-y-2">
                          <p className="text-[10px] font-black text-muted-foreground uppercase">L-V (8:00 AM - 8:00 PM)</p>
                          <MultiSelect value={selectedWeek.weekSoportePerson} onChange={v => updateItem(selectedWeek.id, 'weekSoportePerson', v)} options={agents} placeholder="Sin asignar" />
                        </div>
                      )}
                      <div className="space-y-2">
                        <p className="text-[10px] font-black text-amber-600 uppercase">SÁB-DOM {weekendDatesText}</p>
                        <MultiSelect value={selectedWeek.weekendSoportePerson} onChange={v => updateItem(selectedWeek.id, 'weekendSoportePerson', v)} options={agents} placeholder="Sin asignar" />
                      </div>
                      {selectedWeek.isSpecial && (
                        <div className="space-y-2 p-4 bg-red-50 rounded-2xl border border-red-100">
                          <p className="text-[10px] font-black text-red-600 uppercase">Feriado</p>
                          <MultiSelect value={selectedWeek.specialSoporte} onChange={v => updateItem(selectedWeek.id, 'specialSoporte', v)} options={agents} placeholder="Sin asignar" />
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="md:col-span-2 bg-[#fcfcfc] border border-border/40 rounded-[2rem] p-8 space-y-6">
                    <div className="flex items-center gap-3">
                      <div className="p-3 bg-slate-100 text-slate-600 rounded-2xl"><Building2 className="w-6 h-6" /></div>
                      <h3 className="text-xl font-black uppercase tracking-tighter">Estado de Agencia</h3>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <p className="text-[10px] font-black text-amber-600 uppercase">SÁB-DOM {weekendDatesText}</p>
                        <MultiSelect value={selectedWeek.weekendAgenciaPerson} onChange={v => updateItem(selectedWeek.id, 'weekendAgenciaPerson', v)} options={[{id: 'cerrado', name: 'CERRADO'}, ...agents]} placeholder="Estado" />
                      </div>
                      {selectedWeek.isSpecial && (
                        <div className="space-y-2 p-4 bg-red-50 rounded-2xl border border-red-100">
                          <p className="text-[10px] font-black text-red-600 uppercase">Feriado</p>
                          <MultiSelect value={selectedWeek.specialAgencia} onChange={v => updateItem(selectedWeek.id, 'specialAgencia', v)} options={[{id: 'cerrado', name: 'CERRADO'}, ...agents]} placeholder="Estado" />
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="flex flex-col items-center justify-center h-full p-20 text-center bg-muted/5">
              ...
              <p className="text-muted-foreground font-medium max-w-sm mt-4">
                Selecciona una semana de la lista lateral para gestionar los turnos de guardia.
              </p>
            </div>
          )}
        </section>
      ...
      {deleteModal.isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
           ...
           <div className="bg-white rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl relative z-10 border border-border/40 animate-in zoom-in-95 slide-in-from-bottom-8 duration-500">
              ...
              <h3 className="text-2xl font-black text-center text-foreground uppercase tracking-tight mb-4">Â¿Eliminar Guardia?</h3>
              <p className="text-muted-foreground text-center font-medium mb-10">
                Esta acciÃ³n eliminarÃ¡ permanentemente la planificaciÃ³n de esta semana. No podrÃ¡s deshacer este cambio.
              </p>
        ...
        <div id="pdf-report-content" style={{ 
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '3px solid #2563eb', paddingBottom: '15px', marginBottom: '20px' }}>
             ...
             <div style={{ textAlign: 'right' }}>
                ...
                <p style={{ margin: 2, fontSize: '12px', fontWeight: 700, color: '#64748b' }}>{MONTHS[currentMonth].toUpperCase()} {currentYear} â€¢ SISPROT G.F</p>
             </div>
          </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', border: '2px solid #0f172a' }}>
             <thead>
                <tr style={{ background: '#f1f5f9' }}>
                   <th style={{ border: '1px solid #0f172a', padding: '10px', fontSize: '10px', fontWeight: 900, textTransform: 'uppercase' }}>DÃ­a / Fecha</th>
                   ...
                   <th style={{ border: '1px solid #0f172a', padding: '10px', fontSize: '10px', fontWeight: 900, textTransform: 'uppercase' }}>Soporte TÃ©cnico</th>
                  ...
                  if (w.endDay >= 6) {
                    items.push(
                      <tr key={`${w.id}-we`} style={{ background: '#fffbeb' }}>
                        <td style={{ border: '1px solid #0f172a', padding: '10px', fontWeight: 900, textAlign: 'center', fontSize: '11px' }}>SÃ¡bado {Math.max(w.startDay, 6)} y Domingo {w.endDay}</td>
                        <td style={{ border: '1px solid #0f172a', padding: '10px', fontWeight: 700, fontSize: '11px' }}>
