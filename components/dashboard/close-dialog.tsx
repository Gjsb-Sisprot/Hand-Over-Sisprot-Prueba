...
import { Loader2, CheckCircle, TicketCheck } from "lucide-react";
...
export function CloseDialog({
  ...
  const handleConfirm = async () => {
    ...
    setIsLoading(true);
    const hasExistingGlpiTicket = !!conversation.glpiTicketId;
    const selectedType = TICKET_TYPE_OPTIONS.find((opt) => opt.value === ticketTypeId);
    try {
      await onConfirm(resolution.trim(), {
        createTicket: true,
        ticketTypeId:
          !hasExistingGlpiTicket && ticketTypeId
            ? parseInt(ticketTypeId, 10)
            : undefined,
        ticketTypeName: !hasExistingGlpiTicket ? selectedType?.label : undefined,
        ticketSummary: resolution.trim(),
      });
      setResolution("");
      setTicketTypeId("");
    } finally {
  ...
  const hasExistingGlpiTicket = !!conversation.glpiTicketId;
  const isValid = resolution.trim().length >= 10;
  const canSubmit = hasExistingGlpiTicket
    ? isValid
    : isValid && ticketTypeId !== "";
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Indica cómo se resolvió el caso. Se creará un ticket GLPI para
            dejar constancia.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          {!hasExistingGlpiTicket && (
            <div className="space-y-2">
              <Label htmlFor="close-ticket-type">
                Tipo de Ticket <span className="text-destructive">*</span>
              </Label>
              <select
                id="close-ticket-type"
                value={ticketTypeId}
          ...
          {hasExistingGlpiTicket && (
            <div className="p-3 rounded-lg bg-muted/50">
              <p className="text-xs text-muted-foreground">
                Esta conversación ya tiene ticket GLPI #{conversation.glpiTicketId}. Al cerrar, se agregará la resolución al ticket existente.
              </p>
            </div>
          )}

          <div className="space-y-2">
            <Label htmlFor="close-resolution">
              Resolución <span className="text-destructive">*</span>
            </Label>
            <Input
              id="close-resolution"
              placeholder="Describe cómo se resolvió el caso (mín. 10 caracteres)"
              value={resolution}
            ...
            />
            {resolution.length > 0 && !isValid && (
              <p className="text-xs text-destructive">
                Mínimo 10 caracteres ({resolution.trim().length}/10)
              </p>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-blue-800 dark:text-blue-200">
                Se actualizará ticket GLPI
              </p>
              <p className="text-xs text-blue-600 dark:text-blue-400">
                Deja constancia con la resolución, datos del cliente y resumen
              </p>
            </div>
          </div>
        </div>
