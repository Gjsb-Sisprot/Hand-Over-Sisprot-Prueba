...
export function CalendarView({ technicians }: CalendarViewProps) {
  ...
  const filteredTechnicians = useMemo(() => {
    const areaToMatch = currentCategory === 'support' ? 'Soporte Técnico' : 'Administración';
    return technicians.filter(t => t.area === areaToMatch);
  }, [technicians, currentCategory]);
  ...
  }, [visits, selectedTechnician, currentCategory]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-wrap gap-3">
            <div className="relative group">
              <Filter className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-sidebar-foreground/40 group-focus-within:text-red-500 transition-colors" />
              <select
                value={selectedTechnician}
                onChange={(e) => setSelectedTechnician(e.target.value)}
                className="pl-10 pr-4 py-2 bg-sidebar-background/50 border border-sidebar-border/50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all appearance-none cursor-pointer hover:bg-sidebar-background/80"
              >
